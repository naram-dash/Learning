use std::cell::RefCell;
use std::rc::Rc;

fn main() {
    println!("Hello, world!");

    {
        let x = 5;
        // compile time error
        //  ^^^^^^ cannot borrow `x` as mutable, as it is not declared as mutable
        // let y = &mut x;
    }

    {
        #[derive(Debug)]
        enum List {
            Cons(Rc<RefCell<i32>>, Rc<List>),
            Nil,
        }

        use List::{Cons, Nil};

        let value = Rc::new(RefCell::new(5));

        let a = Rc::new(Cons(Rc::clone(&value), Rc::new(List::Nil)));

        let b = Cons(Rc::new(RefCell::new(3)), Rc::clone(&a));
        let c = Cons(Rc::new(RefCell::new(4)), Rc::clone(&a));

        // (*value).borrow_mut() += 10;
        *(value.borrow_mut()) += 10;
        // 이시점에서 anonymous &mut은 바로 사라짐
        *value.borrow_mut() += 10;

        // let typecheck: RefCell<i32> = *value;
        // let typecheck: std::cell::RefMut<'_, i32> = value.borrow_mut();

        println!("a after = {:?}", a);
        println!("b after = {:?}", b);
        println!("c after = {:?}", c);
    }
}

pub trait Messenger {
    fn send(&self, msg: &str);
}

pub struct LimitTracker<'a, T: Messenger> {
    messenger: &'a T,
    value: usize,
    max: usize,
}

impl<'a, T> LimitTracker<'a, T>
where
    T: Messenger,
{
    pub fn new(messenger: &'a T, max: usize) -> Self {
        LimitTracker {
            messenger,
            value: 0,
            max,
        }
    }

    pub fn set_value(&mut self, value: usize) {
        self.value = value;

        let percentage_of_max = self.value as f64 / self.max as f64;

        if percentage_of_max >= 1.0 {
            self.messenger.send("Error: You are over your quota!");
        } else if percentage_of_max >= 0.9 {
            self.messenger
                .send("Urgent warning: You've used up over 90% of your quota!");
        } else if percentage_of_max >= 0.75 {
            self.messenger
                .send("Warning: You've used up over 75% of your quota!");
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use std::cell::RefCell;

    struct MockMessenger {
        sent_messages: RefCell<Vec<String>>,
    }

    impl MockMessenger {
        fn new() -> Self {
            MockMessenger {
                sent_messages: RefCell::new(Vec::new()),
            }
        }
    }

    impl Messenger for MockMessenger {
        fn send(&self, msg: &str) {
            // cannot borrow `self.sent_messages` as mutable, as it is behind a `&` reference
            //
            // method signature
            // pub fn push(&mut self, value: T) {
            // self.sent_messages.push(String::from(msg));

            self.sent_messages.borrow_mut().push(String::from(msg));
            // pub fn borrow_mut(&self) -> RefMut<'_, T> {
            // pub fn get_mut(&mut self) -> &mut T {

            // runtime error
            // 'already borrowed: BorrowMutError', src/main.rs:85:53
            // because it has two mutable borrows
            let mut one_borrow = self.sent_messages.borrow_mut();
            // let mut two_borrow = self.sent_messages.borrow_mut();
            one_borrow.push(String::from(msg));
            // two_borrow.push(String::from(msg));
        }
    }

    #[test]
    fn it_sends_an_over_75_percent_warn_msg() {
        let mock_messenger = MockMessenger::new();
        let mut limit_tracker = LimitTracker::new(&mock_messenger, 100);

        limit_tracker.set_value(80);

        assert_eq!(mock_messenger.sent_messages.borrow().len(), 1);
    }
}
