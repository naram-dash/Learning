...
let fileSystem account message =
    Directory.CreateDirectory(sprintf @"C:\temp\learnfsharp\lesson14\%s" account.Owner.Name)
    |> ignore

    let filePath =
        sprintf @"C:\temp\learnfsharp\lesson14\%s\%O.txt" account.Owner.Name account.AccountId
