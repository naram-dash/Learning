...
let auditAs operationName audit operation amount account =
    ...
    if accountIsUnchanged then
        audit account (sprintf "%O: Transaction rejected!" DateTime.UtcNow)
    else
        audit account (sprintf "%O: Transaction accepted! Balance is now £%M." DateTime.UtcNow updatedAccount.Balance)
